...
import { isAdminEmail, isSupportAdminEmail } from "../lib/adminConfig";
import {
  getHeroAvailability,
  getHeroBalance,
...
function countryFromCode(
  code: string,
  live?: { count?: number; cost?: number },
): Country {
  const fallback = fallbackCountries.find((item) => item.code === code);
  if (fallback)
    return {
      ...fallback,
      available: live?.count ?? fallback.available,
      startingPrice: live?.cost ?? fallback.startingPrice,
    };
  const normalized = code.toUpperCase();
  const countryName = /^[A-Z]{2}$/.test(normalized)
    ? countryDisplayNames.of(normalized)
    : null;
  return {
    code: normalized,
    name:
      ...
      (normalized.startsWith("H")
        ? `Hero country ${normalized.slice(1)}`
        : normalized),
...
function serviceFromCode(
  ...
  if (fallback)
    return {
      ...fallback,
      available: live?.count ?? fallback.available,
      price: live?.cost ?? fallback.price,
    };
  return {
    code,
...
async function withFastFallback<T>(
  promise: Promise<T>,
  fallback: T,
  timeoutMs = 1000,
): Promise<T> {
...
async function structuralServiceFallback(): Promise<Service[]> {
  ...
  return Array.from(enabledCodes).map((code) =>
    serviceFromCode(code, { count: 0, cost: 0 }),
  );
}
...
async function liveServices(countryCode?: string): Promise<Service[]> {
  ...
  for (const item of catalog) {
    ...
    const current = totals.get(item.serviceCode) ?? {
      count: 0,
      cost: item.cost,
    };
...
async function liveCountries(): Promise<Country[]> {
  ...
  for (const item of catalog) {
    ...
    const current = totals.get(item.countryCode) ?? {
      count: 0,
      cost: item.cost,
    };
    current.count += item.count;
    if (item.cost > 0 && (current.cost === 0 || item.cost < current.cost))
...
function providerStatus(name: "Hero SMS" | "OxaPay") {
  const configured =
    name === "Hero SMS"
      ? Boolean(process.env.HERO_SMS_API_KEY)
      : Boolean(process.env.OXAPAY_MERCHANT_API_KEY);
  return {
    name,
...
async function heroProviderStatus() {
  ...
  try {
    ...
    return {
      ...
      message:
        error instanceof Error ? error.message : "Hero SMS connection failed.",
    };
  }
}
...
async function listServicePrices() {
  const result = await pool.query(
    "SELECT service_code, price FROM sim_service_prices",
  );
  return new Map(
    result.rows.map((row) => [String(row.service_code), Number(row.price)]),
  );
}

async function listCountryServicePrices(countryCode: string) {
  const result = await pool.query(
    "SELECT service_code, price FROM sim_service_country_prices WHERE country_code = $1",
    [countryCode],
  );
  return new Map(
    result.rows.map((row) => [String(row.service_code), Number(row.price)]),
...
async function getCountryBasePrice(
  countryCode: string,
): Promise<number | null> {
  const result = await pool.query(
    "SELECT base_price FROM sim_country_base_prices WHERE country_code = $1",
    [countryCode],
  );
...
async function listAllCountryBasePrices(): Promise<Map<string, number>> {
  const result = await pool.query(
    "SELECT country_code, base_price FROM sim_country_base_prices",
  );
  return new Map(
    result.rows.map((row) => [
...
async function getEffectiveMargin(
  serviceCode: string,
  countryCode?: string,
): Promise<number> {
  if (countryCode) {
...
async function listAllServiceMargins(): Promise<
  Map<string, { global: number | null; byCountry: Map<string, number> }>
> {
  const result = await pool.query(
    "SELECT service_code, country_code, margin_percent FROM sim_service_margins",
  );
  const map = new Map<
    string,
    { global: number | null; byCountry: Map<string, number> }
  >();
...
async function listEnabledServiceCodes() {
  const result = await pool.query(
    "SELECT service_code FROM sim_enabled_services WHERE enabled = TRUE",
  );
...
async function getServicePrice(
  ...
  if (countryResult.rows[0])
    configuredPrice = Number(countryResult.rows[0].price);
  ...
  if (configuredPrice === null) {
    ...
    if (globalResult.rows[0])
      configuredPrice = Number(globalResult.rows[0].price);
  }
  ...
  if (heroSMSCost > 0) {
    const minimumSafePrice = applyMargin(
      heroSMSCost,
...
async function servicesWithPrices(country?: Country, enabledOnly = true) {
  ...
  const countryPrices = country
    ? await listCountryServicePrices(country.code)
  ...
  return services
    .filter((service) => !enabledCodes || enabledCodes.has(service.code))
    .map((service) => {
      const heroSMSCost = service.price;
      ...
      if (countryPrices.has(service.code)) {
        ...
        const countryMargin = country
          ? (serviceMargins?.byCountry.get(country.code) ?? null)
          : null;
        const globalMargin = serviceMargins?.global ?? null;
        const effectiveMargin =
          countryMargin ?? globalMargin ?? DEFAULT_MARGIN_PERCENT;
...
async function createOxaPayInvoice(
  ...
  if (!response.ok || !payload || payload.result !== 100 || !payload.payLink) {
    throw new Error(
      payload?.message || "OxaPay did not return a checkout link.",
    );
  }
...
async function getCoinMerchantKey(coin: string): Promise<string | null> {
  try {
    ...
    return result.rows[0]?.merchant_key
      ? String(result.rows[0].merchant_key)
      : null;
  } catch {
    return null;
  }
...
async function syncExpiredRentals(userId?: string) {
  ...
  for (const row of result.rows) {
    await pool.query(
      "UPDATE sim_rentals SET status = 'expired' WHERE id = $1",
      [row.id],
    );
...
async function syncHeroStatus(id: string) {
  const rentalResult = await pool.query(
    "SELECT * FROM sim_rentals WHERE id = $1",
    [id],
  );
  ...
  if (
    String(rental.status) === "active" &&
    new Date(String(rental.expires_at)) <= new Date()
  ) {
    await pool.query(
      "UPDATE sim_rentals SET status = 'expired' WHERE id = $1",
      [id],
    ...
    try {
      ...
      if (status.status === "STATUS_OK" && status.code) {
        const existing = await pool.query(
          "SELECT COUNT(*)::int AS count FROM sim_sms_messages WHERE rental_id = $1 AND code = $2",
          [id, status.code],
        );
        if (Number(existing.rows[0].count) === 0) {
          await pool.query(
            ...
            [
              crypto.randomUUID(),
              id,
              `Your verification code is ${status.code}.`,
              extractCode(status.code),
            ],
          );
        }
        await setHeroStatus(String(rental.provider_activation_id), 6).catch(
          () => null,
        );
        await pool.query(
          "UPDATE sim_rentals SET status = 'sms_received' WHERE id = $1",
          [id],
        );
      } else if (
        ["STATUS_CANCEL", "NO_ACTIVATION", "STATUS_FINISH"].includes(
          status.status,
        )
      ) {
        await pool.query(
  ...
  const updated = await pool.query("SELECT * FROM sim_rentals WHERE id = $1", [
    id,
  ]);
...
async function listUserRentals(userId: string) {
  ...
  for (const rental of rentalsResult.rows.filter(
  ...
  return syncedRentalsResult.rows.map((row) =>
    mapRental(
      row,
      messagesResult.rows.filter((message) => message.rental_id === row.id),
    ),
  );
}
...
router.get("/status", async (_req, res) => {
  ...
  pool
    .query(
      "DELETE FROM sim_status_metrics WHERE recorded_at < NOW() - INTERVAL '90 days'",
    )
    .catch(() => null);
  ...
  try {
    ...
    activeIncidents = result.rows.map((r) => ({
...
router.post("/admin/status/incidents", async (req, res) => {
  if (
    !req.isAuthenticated() ||
    !isAdminEmail((req.user as { email?: string })?.email ?? "")
  ) {
  ...
  const safeStatus = validStatuses.includes(String(incStatus))
    ? String(incStatus)
    : "investigating";
  const safeComponents = Array.isArray(components)
    ? (components as string[]).filter((c) =>
        (STATUS_COMPONENTS as readonly string[]).includes(c),
      )
    : [];
  const id = crypto.randomUUID();
  const resolvedAt =
    safeStatus === "resolved" ? new Date().toISOString() : null;
  const result = await pool.query(
...
router.post("/admin/status/incidents/:id/updates", async (req, res) => {
  ...
  if (safeStatus === "resolved") {
    ...
    await pool.query(
      "UPDATE sim_status_incidents SET status = $1 WHERE id = $2",
      [safeStatus, id],
    );
  }
  res.json({ ok: true });
...
router.patch("/admin/status/incidents/:id", async (req, res) => {
  ...
  if (!validStatuses.includes(String(incStatus))) {
...
router.delete("/admin/status/incidents/:id", async (req, res) => {
  ...
  await pool.query("DELETE FROM sim_status_incidents WHERE id = $1", [
    req.params.id,
  ]);
  res.json({ ok: true });
});
...
router.use(async (req, res, next) => {
  ...
  try {
    const result = await pool.query(
      "SELECT reason FROM sim_ip_bans WHERE ip_address = $1",
      [ip],
    );
    if (result.rows[0]) {
      res.status(403).json({
...
router.get("/me", async (req, res) => {
  ...
  res.json({
    ...
    suspensionReason: accountData.suspensionReason,
  });
});
...
router.get("/dashboard", async (req, res) => {
  ...
  const payments = await pool.query(
    "SELECT * FROM sim_payments WHERE user_id = $1 ORDER BY created_at DESC LIMIT 5",
    [userId],
  );
  const heroStatus = await heroProviderStatus().catch(() =>
    providerStatus("Hero SMS"),
  );
  const data = GetDashboardResponse.parse({
    account,
    activeRentals: rentals.filter(
      (r) => r.status === "active" || r.status === "sms_received",
    ).length,
...
router.get("/catalog/countries-for-service", async (req, res) => {
  ...
  const allResults = await withFastFallback(
    Promise.all(codesToTry.map((c) => getHeroCountriesForService(c))).then(
      (arrays) => {
  ...
  const mapped = allResults
    .map((live) => ({
      country: countryFromCode(live.countryCode, live),
      available: live.count,
      heroPrice: live.cost,
      activationMinutes: live.activationMinutes ?? 20,
    }))
    .filter((c) => c.available > 0)
  ...
  const result = await Promise.all(
    mapped.map(
      async ({ country, available, heroPrice, activationMinutes }) => ({
        code: country.code,
        name: country.name,
        flag: country.flag,
        available,
...
router.get("/catalog/services", async (req, res) => {
  ...
  res.json(
    ListServicesResponse.parse({
      services: await servicesWithPrices(country),
      provider: providerStatus("Hero SMS"),
    }),
  );
});
...
router.get("/catalog/availability", async (req, res) => {
  ...
  const live = await withFastFallback(
    getHeroAvailability(params.serviceCode, params.countryCode),
    null,
  );
...
router.get("/rentals", async (req, res) => {
  res.json(
    ListRentalsResponse.parse({
      rentals: await listUserRentals(getUserId(req)),
    }),
  );
});
...
router.post("/rentals", async (req, res) => {
  ...
  const live = await getHeroAvailability(
    body.serviceCode,
    body.countryCode,
  ).catch(() => null);
  ...
  const id = crypto.randomUUID();
  try {
    const providerRental = await rentHeroNumber(
      service.code,
      country.code,
      price || undefined,
    );
    ...
    const expiresAt = new Date(
      Date.now() + activationMinutes * 60_000,
    ).toISOString();
    const result = await pool.query(
      ...
      [
        ...
        service.code,
        service.name,
        providerRental.phoneNumber,
        price,
        providerRental.activationId,
        activationMinutes,
        expiresAt,
      ],
    );
    ...
    res.status(502).json({
      error:
        error instanceof Error
          ? error.message
          : "Hero SMS could not allocate a number.",
    });
  }
});
...
router.post("/rentals/:id/refresh", async (req, res) => {
  ...
  const messages = await pool.query(
    "SELECT * FROM sim_sms_messages WHERE rental_id = $1 ORDER BY received_at DESC",
    [params.id],
  );
...
router.post("/rentals/:id/cancel", async (req, res) => {
  ...
  const rental = await pool.query("SELECT * FROM sim_rentals WHERE id = $1", [
    params.id,
  ]);
  ...
  if (row.provider_activation_id) {
    const providerResponse = await setHeroStatus(
      String(row.provider_activation_id),
      8,
    );
    if (providerResponse === "EARLY_CANCEL_DENIED") {
      res.status(409).json({
        error:
          "Hero SMS allows cancellation only after the provider's minimum waiting time. Try again shortly.",
      });
      return;
    }
  }
  await pool.query(
    "UPDATE sim_rentals SET status = 'cancelled' WHERE id = $1",
    [params.id],
  );
  await refundExpiredRental(row);
  ...
  res.json(
    CancelRentalResponse.parse(mapRental(updatedRental.rows[0], messages.rows)),
  );
});

router.get("/payments", async (req, res) => {
  const result = await pool.query(
    "SELECT * FROM sim_payments WHERE user_id = $1 ORDER BY created_at DESC",
    [getUserId(req)],
  );
  res.json(
  ...
  try {
    ...
    if (status === "Paid") {
      if (String(payment.status) !== "paid") {
        ...
        if (credits > 0) {
          ...
          if (lienAmount > 0) {
            if (credits >= lienAmount) {
              ...
              await pool.query(
                "UPDATE sim_users SET lien_amount = 0 WHERE id = $1",
                [payment.user_id],
              );
            } else {
              creditsToAdd = 0;
        ...
        try {
          const refSettingsRow = await pool.query(
            "SELECT min_deposit_amount FROM sim_referral_settings WHERE id = 1",
          );
          const minDeposit = Number(
            refSettingsRow.rows[0]?.min_deposit_amount ?? 0,
          );
          if (minDeposit > 0 && Number(payment.amount) >= minDeposit) {
            ...
            if (pendingRef.rows[0]) {
              ...
              await pool.query(
                "UPDATE sim_referrals SET credited = TRUE WHERE id = $1",
                [ref.id],
              );
              await pool.query(
                "UPDATE sim_users SET credits = credits + $1 WHERE id = $2",
              ...
              await pool.query(
                ...
                [
                  ...
                  `Your referred friend made their first deposit. You both received $${bonus.toFixed(2)} credit!`,
                ],
              );
              await pool.query(
                `INSERT INTO sim_notifications (user_id, title, message, type)
                ...
                [
                  ...
                  `Your $${bonus.toFixed(2)} referral bonus has been credited to your account!`,
                ],
              );
            }
          }
...
router.post("/payments/checkout", async (req, res) => {
  ...
  const couponCodeRaw =
    typeof (req.body as any).couponCode === "string"
      ? (req.body as any).couponCode.trim().toUpperCase()
      : null;
  const selectedCoin =
    typeof (req.body as any).coin === "string"
      ? (req.body as any).coin.toUpperCase().trim()
      : null;
  ...
  try {
    ...
    if (couponCodeRaw) {
      ...
      const validCoupon =
        ...
        (coupon.max_uses === null ||
          Number(coupon.uses_count) < Number(coupon.max_uses)) &&
        (!coupon.target_user_email || coupon.target_user_email === userEmail);

      if (validCoupon) {
        if (coupon.type === "percentage") {
          discountAmount = Number(
            (body.amount * (Number(coupon.value) / 100)).toFixed(2),
          );
        } else {
    ...
    const coinMerchantKey = selectedCoin
      ? await getCoinMerchantKey(selectedCoin)
      : null;
    ...
    const chargedAmount = Number(
      Math.max(body.amount - discountAmount, 0.01).toFixed(2),
    );
    const totalCredits = body.amount;
    const { payLink: checkoutUrl, trackId } = await createOxaPayInvoice(
      req,
      id,
      chargedAmount,
      fiatCurrency,
      totalCredits,
      coinMerchantKey || undefined,
      payCurrency,
    );
    const result = await pool.query(
    ...
    res.status(502).json({
      error:
        error instanceof Error
          ? error.message
          : "Unable to create OxaPay checkout.",
    });
  }
});
...
router.post("/coupons/validate", async (req, res) => {
  try {
    ...
    if (coupon.target_user_email && coupon.target_user_email !== userEmail) {
      res
        .status(403)
        .json({ error: "This coupon is not valid for your account." });
      return;
    }
    res.json({
      valid: true,
...
router.post("/admin/coupons", async (req, res) => {
  if (!(await requireAdmin(req, res))) return;
  try {
    const { code, type, value, maxUses, targetUserEmail, expiresAt } =
      req.body as {
        code: string;
        type: string;
        value: number;
...
router.get("/admin/overview", async (req, res) => {
  ...
  const users = await pool.query(
    "SELECT COUNT(*)::int AS count FROM sim_users",
  );
  const rentals = await pool.query(
    "SELECT COUNT(*)::int AS count FROM sim_rentals WHERE status IN ('active', 'sms_received')",
  );
  const revenue = await pool.query(
    "SELECT COALESCE(SUM(amount), 0)::numeric AS total FROM sim_payments WHERE status = 'paid'",
  );
  const pending = await pool.query(
    "SELECT COUNT(*)::int AS count FROM sim_payments WHERE status = 'pending'",
  );
...
router.put("/admin/users/:id/role", async (req, res) => {
  ...
  if (role !== "admin" && role !== "support_admin" && role !== "user") {
    res
      .status(400)
      .json({ error: "Role must be 'admin', 'support_admin', or 'user'." });
    return;
  }
  const result = await pool.query(
...
router.put("/admin/users/:id/status", async (req, res) => {
  ...
  if (status !== "active" && status !== "suspended" && status !== "banned") {
    res
      .status(400)
      .json({ error: "Status must be 'active', 'suspended', or 'banned'." });
    return;
  }
...
router.get("/admin/services", async (req, res) => {
  ...
  const countryCode = String(req.query.countryCode ?? "");
  const country = countryCode ? countryFromCode(countryCode) : undefined;
  const countryOverrides = country
    ? await listCountryServicePrices(country.code)
    : new Map<string, number>();
  ...
  for (const item of fullCatalog) {
    ...
    const cCurrent = allCountryTotals.get(item.countryCode) ?? {
      count: 0,
      cost: item.cost,
    };
    cCurrent.count += item.count;
    if (item.cost > 0 && (cCurrent.cost === 0 || item.cost < cCurrent.cost))
      cCurrent.cost = item.cost;
    ...
    const sCurrent = serviceTotals.get(item.serviceCode) ?? {
      count: 0,
      cost: item.cost,
    };
  ...
  const activeServices = Array.from(serviceTotals.entries()).map(
    ([code, live]) => serviceFromCode(code, live),
  );
  ...
  res.json({
    ...
    countries: allCountries.map((c) => ({
      ...c,
      customBasePrice: countryBasePrices.has(c.code)
        ? countryBasePrices.get(c.code)
        : null,
    })),
    enabledServiceCodes: Array.from(
      enabledServiceCodes ??
        new Set(activeServices.map((service) => service.code)),
    ),
    ...
    services: allKnownServices.map((service) => {
      ...
      return {
        ...
        price,
        customPrice:
          countryOverrides.has(service.code) ||
          priceOverrides.has(service.code),
        countryPrice: countryOverrides.has(service.code)
          ? Number(countryOverrides.get(service.code))
          : null,
        globalPrice: priceOverrides.has(service.code)
          ? Number(priceOverrides.get(service.code))
          : null,
        globalMargin,
        countryMargin,
...
router.put("/admin/countries/:code/base-price", async (req, res) => {
  ...
  if (price === 0) {
    await pool.query(
      "DELETE FROM sim_country_base_prices WHERE country_code = $1",
      [code],
    );
    res.json({
      countryCode: code,
      basePrice: null,
      message: "Custom base price removed, will use live API price.",
    });
    return;
  }
  await pool.query(
...
router.put("/admin/services/:code/price", async (req, res) => {
  ...
  const countryCode =
    typeof req.body?.countryCode === "string"
      ? String(req.body.countryCode)
      : "";
  const service =
    (await liveServices(countryCode || undefined)).find(
      (item) => item.code === code,
    ) ?? serviceFromCode(code);
  if (!service) {
...
router.delete("/admin/services/:code/price", async (req, res) => {
  ...
  if (countryCode) {
    await pool.query(
      "DELETE FROM sim_service_country_prices WHERE service_code = $1 AND country_code = $2",
      [code, countryCode],
    );
  } else {
    await pool.query("DELETE FROM sim_service_prices WHERE service_code = $1", [
      code,
    ]);
  }
  res.json({
    ...
    code,
    countryCode: countryCode || null,
    message: "Fixed price removed. Margin-based pricing will apply.",
  });
});
...
router.put("/admin/services/:code/margin", async (req, res) => {
  ...
  if (!Number.isFinite(margin) || margin < 0 || margin > 10000) {
    res
      .status(400)
      .json({ error: "Margin must be between 0 and 10000 percent." });
    return;
  }
  await pool.query(
...
router.delete("/admin/services/:code/margin", async (req, res) => {
  ...
  res.json({
    ...
    code,
    countryCode: countryCode || null,
    message: "Margin reset to default.",
  });
});
...
router.get("/admin/hero-catalog-codes", async (req, res) => {
  ...
  try {
    ...
    const totals = new Map<
      string,
      { count: number; knownName: string | null }
    >();
    for (const item of catalog) {
      const current = totals.get(item.serviceCode) ?? {
        count: 0,
        knownName: serviceNames[item.serviceCode]?.name ?? null,
      };
      current.count += item.count;
      totals.set(item.serviceCode, current);
    }
    ...
    res.status(502).json({
      error:
        err instanceof Error
          ? err.message
          : "Failed to fetch Hero SMS catalog.",
    });
  }
});

async function ticketMessages(
  ticketId: string,
  ticketRow: Record<string, unknown>,
) {
  const result = await pool.query(
    "SELECT * FROM sim_support_messages WHERE ticket_id = $1 ORDER BY created_at ASC",
    [ticketId],
  );
...
async function findSupportTicket(reference: string, userId?: string) {
  const params = userId ? [reference, userId] : [reference];
  const condition = userId
    ? "(id = $1 OR ticket_number = $1) AND user_id = $2"
    : "id = $1 OR ticket_number = $1";
  const result = await pool.query(
    `SELECT * FROM sim_support_tickets WHERE ${condition}`,
    params,
  );
...
async function mapSupportTicket(
  ...
  return {
    ...
    displayId: row.ticket_number ? String(row.ticket_number) : String(row.id),
    ...(viewer === "admin"
      ? {
...
router.get("/support/tickets", async (req, res) => {
  ...
  res.json({
    tickets: await Promise.all(
      result.rows.map((row) => mapSupportTicket(row, "user")),
    ),
  });
});
...
router.post("/support/tickets", async (req, res) => {
  ...
  const { subject, category, priority, message } = req.body as Record<
    string,
    string
  >;
  if (!subject?.trim() || !category?.trim() || !message?.trim()) {
    res
      .status(400)
      .json({ error: "Subject, category, and message are required." });
    return;
  }
  ...
  const id = crypto.randomUUID();
  const result = await pool.query(
    `INSERT INTO sim_support_tickets (id, ticket_number, user_id, subject, category, priority, message)
     VALUES ($1, 'TKT-' || LPAD(nextval('sim_support_ticket_number_seq')::text, 6, '0'), $2, $3, $4, $5, $6) RETURNING *`,
    [
      id,
      getUserId(req),
...
router.post("/support/tickets/:id/messages", async (req, res) => {
  ...
  const imageData =
    typeof req.body?.imageData === "string"
      ? req.body.imageData.slice(0, 2_000_000)
      : null;
  if (!message && !imageData) {
    res.status(400).json({ error: "Message or image is required." });
    return;
  }
  const ticket = await findSupportTicket(id, getUserId(req));
  if (!ticket) {
  ...
  if (["resolved", "closed"].includes(String(ticket.status))) {
    res.status(409).json({
      error:
        "This ticket is resolved or closed. Please open a new ticket if you still need help.",
      ended: true,
    });
    return;
  }
  ...
  await pool.query(
    "UPDATE sim_support_tickets SET status = 'open', updated_at = NOW() WHERE id = $1",
    [ticketId],
  );
  const updated = await findSupportTicket(id, getUserId(req));
  res.json({ ticket: await mapSupportTicket(updated!, "user") });
});
...
router.get("/support/tickets/:id", async (req, res) => {
  ...
  res.json({ ticket: await mapSupportTicket(ticket, "user") });
});
...
router.get("/admin/support", async (req, res) => {
  ...
  res.json({
    tickets: await Promise.all(
      result.rows.map((row) => mapSupportTicket(row, "admin")),
    ),
  });
});
...
router.patch("/admin/support/:id", async (req, res) => {
  ...
  const existingTicket = await findSupportTicket(id);
  if (!existingTicket) {
    res.status(404).json({ error: "Ticket not found." });
    return;
  }
...
router.post("/keys", async (req, res) => {
  ...
  if (count.rows[0].n >= 3)
    return res.status(400).json({
      error:
        "Maximum 3 active API keys per account. Revoke one to create another.",
    });
  ...
  return res
    .status(201)
    .json({ id, name, prefix, key: full, createdAt: new Date().toISOString() });
});
...
async function ensureReferralCode(userId: string): Promise<string> {
  const existing = await pool.query(
    "SELECT referral_code FROM sim_users WHERE id = $1",
    [userId],
  );
  if (existing.rows[0]?.referral_code)
    return String(existing.rows[0].referral_code);
  // Generate unique code
  let code = generateReferralCode();
  for (let attempt = 0; attempt < 10; attempt++) {
    const conflict = await pool.query(
      "SELECT id FROM sim_users WHERE referral_code = $1",
      [code],
    );
    if (conflict.rowCount === 0) break;
    code = generateReferralCode();
  }
  await pool.query("UPDATE sim_users SET referral_code = $1 WHERE id = $2", [
    code,
    userId,
  ]);
  return code;
...
router.get("/referrals", async (req, res) => {
  ...
  const settingsRow = await pool.query(
    "SELECT enabled, bonus_amount, min_deposit_amount FROM sim_referral_settings WHERE id = 1",
  );
  const refSettings = settingsRow.rows[0] ?? {
    enabled: true,
    bonus_amount: 0.5,
...
router.post("/referrals/apply", async (req, res) => {
  ...
  const alreadyReferred = await pool.query(
    "SELECT id FROM sim_referrals WHERE referred_id = $1",
    [userId],
  );
  ...
  const referrer = await pool.query(
    "SELECT id FROM sim_users WHERE UPPER(referral_code) = UPPER($1)",
    [code.trim()],
  );
  if (!referrer.rows[0])
  ...
  if (!refSettings.enabled) {
    return res
      .status(403)
      .json({ error: "The referral program is currently disabled." });
  }
    ...
    if (shouldCreditNow) {
      ...
      await client.query(
        ...
        [
          referrerId,
          "Referral bonus earned!",
          `Someone signed up using your referral code. You both received $${BONUS.toFixed(2)} credit!`,
        ],
      );
      await client.query(
        `INSERT INTO sim_notifications (user_id, title, message, type)
        ...
        [
          ...
          `Your referral code was accepted. $${BONUS.toFixed(2)} credit has been added to your account!`,
        ],
      );
    } else {
      await client.query(
        ...
        [
          ...
          `Make a deposit of at least $${MIN_DEPOSIT.toFixed(2)} to unlock your $${BONUS.toFixed(2)} referral bonus.`,
        ],
      );
    }
    await client.query("COMMIT");
...
router.get("/admin/referral-settings", async (req, res) => {
  ...
  return res.json({
    ...
    minDepositAmount: Number(row.min_deposit_amount),
  });
});
...
router.put("/admin/referral-settings", async (req, res) => {
  ...
  if (!isAdminEmail(user.email))
    return res.status(403).json({ error: "Forbidden" });
  const { enabled, bonusAmount, minDepositAmount } = req.body as {
    enabled?: boolean;
    bonusAmount?: number;
  ...
  const safeMinDeposit =
    typeof minDepositAmount === "number" && minDepositAmount >= 0
      ? minDepositAmount
      : 0;
  await pool.query(
  ...
  return res.json({
    ...
    enabled,
    bonusAmount,
    minDepositAmount: safeMinDeposit,
  });
});
...
router.get("/admin/ip-bans", async (req, res) => {
  if (!(await requireAdmin(req, res))) return;
  const result = await pool.query(
    "SELECT * FROM sim_ip_bans ORDER BY created_at DESC",
  );
  res.json({
...
router.post("/admin/ip-bans", async (req, res) => {
  if (!(await requireAdmin(req, res))) return;
  const { ipAddress, reason } = req.body as {
    ipAddress?: string;
    reason?: string;
  };
  if (!ipAddress?.trim()) {
  ...
  try {
    ...
    if (err?.code === "23505") {
      res.status(409).json({ error: "This IP is already banned." });
    } else {
...
router.delete("/admin/ip-bans/:id", async (req, res) => {
  if (!(await requireAdmin(req, res))) return;
  const result = await pool.query(
    "DELETE FROM sim_ip_bans WHERE id = $1 RETURNING id",
    [req.params.id],
  );
  if (!result.rows[0]) {
...
router.get("/admin/coin-api-keys", async (req, res) => {
  if (!(await requireAdmin(req, res))) return;
  const result = await pool.query(
    "SELECT * FROM sim_coin_api_keys ORDER BY coin",
  );
  res.json({
    keys: result.rows.map((r) => ({
      coin: String(r.coin),
...
router.put("/admin/coin-api-keys/:coin", async (req, res) => {
  ...
  if (!validCoins.includes(coin)) {
    res
      .status(400)
      .json({ error: "Invalid coin. Supported: " + validCoins.join(", ") });
    return;
  }
  const { merchantKey, enabled = true } = req.body as {
    merchantKey?: string;
    enabled?: boolean;
  };
  if (!merchantKey?.trim()) {
    res.status(400).json({ error: "Merchant key is required." });
    return;
  }
  await pool.query(
...
router.delete("/admin/coin-api-keys/:coin", async (req, res) => {
  ...
  const result = await pool.query(
    "DELETE FROM sim_coin_api_keys WHERE coin = $1 RETURNING coin",
    [coin],
  );
  if (!result.rows[0]) {
...
router.get("/admin/users/:id/lien", async (req, res) => {
  if (!(await requireAdmin(req, res))) return;
  const result = await pool.query(
    "SELECT id, name, email, lien_amount FROM sim_users WHERE id = $1",
    [req.params.id],
  );
  if (!result.rows[0]) {
    res.status(404).json({ error: "User not found." });
  ...
  res.json({
    ...
    lienAmount: Number(result.rows[0].lien_amount ?? 0),
  });
});
...
router.post("/admin/support/:id/messages", async (req, res) => {
  ...
  await pool.query(
    "UPDATE sim_support_tickets SET updated_at = NOW() WHERE id = $1",
    [ticketId],
  );
  ...
  async function expireStalePayments() {
    try {
      ...
      if (result.rowCount && result.rowCount > 0) {
        console.log(
          `[payments] Expired ${result.rowCount} stale pending payment(s).`,
        );
      }
    } catch (err) {
