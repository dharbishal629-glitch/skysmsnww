          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-8 pb-16">
              ...
              >
                ...
                {/* Megaphone body */}
                <path
                  d="M28 42 L52 30 L52 70 L28 58 Z"
                  fill="#2a3f5e"
                  stroke="#3a5580"
                  strokeWidth="1.5"
                ...
                {/* Bell of megaphone */}
                <path
                  d="M52 34 Q70 40 70 50 Q70 60 52 66 Z"
                  fill="#2a3f5e"
                  stroke="#3a5580"
                  strokeWidth="1.5"
                  strokeLinejoin="round"
            ...
            <div className="p-3 space-y-1">
              {notifications.map((n) => (
                ...
                >
                  <div
                    className={`h-9 w-9 shrink-0 rounded-xl flex items-center justify-center mt-0.5 ${typeBg(n.type, n.read)}`}
                  >
                    {typeIcon(n.type)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <span
                        className={`text-[13px] font-semibold leading-snug ${n.read ? "text-slate-300" : "text-white"}`}
                      >
                        {n.title}
                      </span>
                    ...
                    <div className="flex items-center gap-1.5 mt-1 mb-0.5">
                      <span
                        className={`inline-flex items-center text-[10px] font-bold px-1.5 py-0.5 rounded-full border ${typeLabel(n.type).cls}`}
                      >
                        {typeLabel(n.type).label}
                      </span>
                    </div>
                    <p
                      className={`text-[12px] leading-relaxed mt-0.5 ${n.read ? "text-slate-500" : "text-slate-400"}`}
                    >
                      {n.message}
                    </p>
                    {n.link && (
    ...
    <div className="app-sidebar-inner flex flex-col h-full overflow-hidden bg-white dark:bg-slate-900">
      ...
      <div className="px-3 pt-3 mb-2 shrink-0">
        ...
        >
          ...
          <span className="text-[12px] flex-1 text-left text-slate-400 dark:text-slate-500">
            {t("search")}
          </span>
          <kbd className="hidden sm:flex h-5 items-center gap-0.5 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-1.5 text-[9px] font-mono text-slate-400 dark:text-slate-500">
            ⌘K
          </kbd>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-1 space-y-0.5 min-h-0 pt-1">
        {mainNavItems.map((item) => {
          ...
          const label = item.labelKey
            ? t(item.labelKey)
            : ((item as { label?: string }).label ?? item.href);
          return (
            <NavItem
              key={item.href}
              href={item.href}
      ...
      <div className="px-3 pb-4 border-t border-slate-100 dark:border-slate-800 pt-3 shrink-0">
        {isLoading ? (
          ...
          <div className="flex items-center gap-2.5 px-1">
            ...
            <div className="flex-1 min-w-0">
              <div
                className="text-[12.5px] font-semibold text-slate-800 dark:text-white truncate"
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="app-shell min-h-screen flex">
      ...
      {/* Desktop sidebar */}
      <aside className="app-sidebar hidden md:flex w-[240px] flex-col fixed inset-y-0 left-0 z-30 border-r border-slate-200 dark:border-slate-800 shadow-sm">
        <SidebarContent
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 flex md:hidden">
          <div
            className={`absolute inset-0 bg-slate-900/30 dark:bg-slate-900/60 backdrop-blur-sm ${closing ? "overlay-fade-out" : "overlay-fade-in"}`}
            onClick={closeSidebar}
          />
          <aside
            className={`app-sidebar relative z-50 w-[260px] flex flex-col h-full border-r border-slate-200 dark:border-slate-800 shadow-2xl ${closing ? "sidebar-slide-out" : "sidebar-slide-in"}`}
          >
            <button
              onClick={closeSidebar}
              className="absolute top-4 right-4 h-8 w-8 flex items-center justify-center rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-150 z-10"
            >
              <X className="h-4 w-4" />
            </button>
            <SidebarContent
      ...
      {/* Main content */}
      <div className="app-content relative z-10 flex-1 min-w-0 flex flex-col md:pl-[240px] page-grid bg-[#f7f9ff] dark:bg-[#080c18]">
        {/* Mobile header */}
        <header className="app-mobile-header md:hidden sticky top-0 z-20 flex items-center gap-3 px-4 h-14 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
          <button
            onClick={openSidebar}
          ...
          <div className="flex items-center gap-1 ml-auto">
            {user && (
              <span className="text-[13px] font-bold text-[#0a1628] dark:text-white">
