...
export default function Notifications() {
  ...
  return (
    <div
      className="min-h-screen bg-white dark:bg-[#0a0f1e] flex flex-col sky-page"
      data-sky-page="notifications"
    >
      {/* ── Header ──────────────────────────────────────────────── */}
        ...
        {loading ? (
          <div className="space-y-3 pt-4">
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className="flex gap-3 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900"
              >
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-[13px] text-slate-400 dark:text-slate-500 mt-1">
                We'll notify you about activity on your account.
              </p>
            </div>
          </div>
        ) : (
          <div className="space-y-2 pt-3">
            {notifications.map((n) => (
              ...
              >
                {/* Icon */}
                <div
                  className={`h-10 w-10 shrink-0 rounded-xl border flex items-center justify-center mt-0.5 ${typeBg(n.type, n.read)}`}
                >
                  {typeIcon(n.type)}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <span
                      className={`text-[13.5px] font-semibold leading-snug ${n.read ? "text-slate-700 dark:text-slate-300" : "text-slate-900 dark:text-white"}`}
                    >
                      {n.title}
                    </span>
                  ...
                  <p
                    className={`text-[12.5px] leading-relaxed mt-0.5 ${n.read ? "text-slate-500 dark:text-slate-400" : "text-slate-600 dark:text-slate-300"}`}
                  >
                    {n.message}
                  </p>
                  {n.link && (
