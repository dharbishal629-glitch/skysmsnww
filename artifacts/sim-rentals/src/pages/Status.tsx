...
function UptimeBars({
  ...
  const bars = Array.from({ length: 90 }, (_, i) => {
    ...
    const hit = incidents.find((inc) => {
      if (inc.components.length && !inc.components.includes(component))
        return false;
    ...
    <div className="relative">
      ...
      >
        ...
        <span
          style={{
            color:
              STATUS_COLOR[status as keyof typeof STATUS_COLOR] ?? "#1b9e3e",
          }}
        >
          {uptimePct}% uptime
        </span>
...
function ResponseChart({
    ...
    return (
      ...
      >
        <span style={{ fontSize: 12, color: "#555" }}>
          Not enough data yet. Check back after more status checks.
        </span>
      </div>
    );
  }
  ...
  const shownLabels = labels.map((l, i) =>
...
export default function StatusPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (iRes.ok) {
        const j = (await iRes.json()) as { incidents: Incident[] };
        setIncidents(j.incidents ?? []);
      }
      if (mRes.ok) {
        const j = (await mRes.json()) as { metrics: MetricDay[] };
  ...
  return (
    <div style={s} className="sky-page" data-sky-page="status">
      {/* Banner */}
      <div
        style={{
      ...
      <div
        style={{ maxWidth: 780, margin: "0 auto", padding: "36px 20px 60px" }}
      >
        {/* Header */}
