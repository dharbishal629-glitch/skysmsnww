        ...
        <div className="p-5">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-bold text-slate-900 dark:text-white text-[14.5px]">
...
export default function AdminUserDetail() {
  ...
  const addCredits = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${amount > 0 ? "Added" : "Removed"} $${Math.abs(amount).toFixed(2)} credits.`,
      });
    } catch (err) {
      toast({
        title: "Failed",
  ...
  const handleLien = async () => {
    ...
    try {
      ...
      toast({
        title: "Lien updated",
        description: `Lien set to $${amount.toFixed(2)}.`,
      });
    } catch (err) {
      toast({
  ...
  return (
    <div
      ...
      data-sky-page="admin-user-detail"
    >
      {suspendModal && data && (
        <SuspendModal
          user={data}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-0.5">
            Manage this account's credits, role, and status.
          </p>
        </div>
      </div>
        ...
        <>
            ...
            <div className="p-5 flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h2 className="font-bold text-slate-900 dark:text-white text-[18px]">
                    {data.name}
                  </h2>
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${isSuspended ? "text-red-400 border-red-500/20 bg-red-500/10" : "text-emerald-400 border-emerald-500/20 bg-emerald-500/10"}`}
                  >
                    {data.status}
                  </span>
                  <span
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              >
                ...
                <div className="text-[17px] font-bold text-slate-900 dark:text-white">
                  {value}
                </div>
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] divide-y divide-white/[0.04]">
            <div className="flex items-center gap-2.5 px-4 py-3">
              ...
              <button
                onClick={() => copyText(data.email, "email")}
                className="h-6 w-6 flex items-center justify-center rounded-md text-slate-700 hover:text-slate-400 transition-colors"
              >
                {copiedField === "email" ? (
            ...
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <span className="text-[11px] text-slate-500 font-mono flex-1 select-all break-all">
                {data.id}
              </span>
              <button
            ...
            {data.lastSeenIp && (
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <span className="text-[13px] text-slate-300 font-mono flex-1">
                  {data.lastSeenIp}
                </span>
                <button
                  onClick={() => copyText(data.lastSeenIp!, "ip")}
                  className="h-6 w-6 flex items-center justify-center rounded-md text-slate-700 hover:text-slate-400 transition-colors"
                >
                  {copiedField === "ip" ? (
                    <Check className="h-3 w-3 text-emerald-400" />
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-3">
            <label className="text-[12px] font-semibold text-slate-400 flex items-center gap-2">
              <DollarSign className="h-3.5 w-3.5 text-emerald-500" /> Adjust
              Credits
            </label>
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-3">
            <label className="text-[12px] font-semibold text-slate-400 flex items-center gap-2">
              ...
              <span className="text-[10.5px] text-slate-700 font-normal">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={handleLien}
                disabled={savingLien}
                className="h-10 px-5 rounded-xl bg-red-600/60 border border-red-500/25 text-[13px] font-bold text-white hover:bg-red-600/80 transition-all disabled:opacity-50 flex items-center gap-1.5"
