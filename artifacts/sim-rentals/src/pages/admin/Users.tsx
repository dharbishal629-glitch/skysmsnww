...
function SuspendModal({
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md rounded-2xl border border-white/[0.08] bg-[#070c1a] shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div
          className={`h-px ${isSuspended ? "bg-emerald-500/40" : "bg-red-500/40"}`}
        />
        <div className="p-5">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              <div
                className={`h-9 w-9 rounded-xl border flex items-center justify-center shrink-0 ${isSuspended ? "bg-emerald-500/10 border-emerald-500/20" : "bg-red-500/10 border-red-500/20"}`}
              >
                {isSuspended ? (
              ...
              <div>
                <div className="font-bold text-white text-[14.5px]">
            ...
            <button
              onClick={onClose}
              className="h-7 w-7 flex items-center justify-center rounded-lg text-slate-600 hover:text-white hover:bg-white/[0.06] transition-all"
            >
          ...
          {!isSuspended && (
            <div className="mb-4">
              <label className="block text-[11.5px] font-semibold text-slate-500 mb-2">
                Reason <span className="text-slate-700">(optional)</span>
              </label>
              <input
                value={reason}
          ...
          <div className="flex gap-2.5">
            <button
              onClick={onClose}
              className="flex-1 h-10 rounded-xl border border-white/[0.08] text-[13px] font-semibold text-slate-500 hover:text-white hover:bg-white/[0.04] transition-all"
            >
              Cancel
            </button>
            <button
...
function UserProfileModal({
  ...
  onRoleChange: (role: "admin" | "support_admin" | "user") => Promise<void>;
}) {
  ...
  const handleLien = async () => {
    ...
    if (!Number.isFinite(amount) || amount < 0) {
      toast({
        title: "Invalid lien amount",
        description: "Enter 0 or greater.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Lien updated",
        description: `Lien set to $${amount.toFixed(2)} — deducted on next top-up.`,
      });
    } catch (err: unknown) {
      toast({
        title: "Failed to set lien",
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/[0.05] sticky top-0 bg-[#070c1a] z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <span
                  className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full border ${isSuspended ? "text-red-400 border-red-500/20 bg-red-500/10" : "text-emerald-400 border-emerald-500/20 bg-emerald-500/10"}`}
                >
                  {user.status}
                </span>
                <span
          ...
          <button
            onClick={onClose}
            className="h-7 w-7 flex items-center justify-center rounded-lg text-slate-600 hover:text-white hover:bg-white/[0.06] transition-all shrink-0"
          >
            <X className="h-3.5 w-3.5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] divide-y divide-white/[0.04]">
            <div className="flex items-center gap-2.5 px-3 py-2.5">
              ...
              <span className="text-[12.5px] text-slate-400 truncate flex-1">
                {user.email}
              </span>
              <button
                onClick={() => copyText(user.email, "email")}
                className="h-6 w-6 flex items-center justify-center rounded-md text-slate-700 hover:text-slate-400 transition-colors shrink-0"
              >
                {copiedField === "email" ? (
            ...
            <div className="flex items-center gap-3 px-3.5 py-2.5">
              ...
              <span className="text-[11px] text-slate-500 font-mono truncate flex-1 select-all">
                {user.id}
              </span>
              <button
            ...
            {user.lastSeenIp && (
              <div className="flex items-center gap-3 px-3.5 py-2.5">
                ...
                <span className="text-[12px] text-slate-400 font-mono flex-1">
                  {user.lastSeenIp}
                </span>
                <button
                  onClick={() => copyText(user.lastSeenIp!, "ip")}
                  className="h-6 w-6 flex items-center justify-center rounded-md text-slate-700 hover:text-slate-400 transition-colors shrink-0"
                >
                  {copiedField === "ip" ? (
                    <Check className="h-3 w-3 text-emerald-400" />
          ...
          <div className="space-y-1.5">
            <label className="text-[11.5px] font-semibold text-slate-500 flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-slate-700 font-normal">
                (deducted from next top-up)
              </span>
            </label>
            <div className="flex gap-2">
              <div className="relative flex-1">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 font-bold text-[12px] pointer-events-none">
                  $
                </span>
                <input
                  type="number"
              ...
              <button
                onClick={handleLien}
                disabled={savingLien}
                className="h-9 px-4 rounded-xl bg-red-600/60 border border-red-500/25 text-[12.5px] font-bold text-white hover:bg-red-600/80 transition-all disabled:opacity-50 flex items-center gap-1.5"
              >
                {savingLien ? (
          ...
          <div className="flex gap-2 pt-0.5">
            ...
            <button
              onClick={onSuspend}
              className={`flex-1 h-9 flex items-center justify-center gap-1.5 rounded-xl text-[12.5px] font-semibold transition-all ${isSuspended ? "border border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-400 hover:bg-emerald-500/[0.1]" : "border border-red-500/20 bg-red-500/[0.06] text-red-400 hover:bg-red-500/[0.1]"}`}
            >
              {isSuspended ? (
                <>
...
export default function AdminUsers() {
  ...
  const addCredits = async (userId: string, amount?: number) => {
    ...
    if (!Number.isFinite(amt) || amt === 0) {
      toast({
        title: "Enter a credit amount",
        description: "Use positive to add, negative to remove.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `${amt > 0 ? "Added" : "Removed"} $${Math.abs(amt).toFixed(2)} credits.`,
      });
    } catch (err) {
      toast({
  ...
  const changeRole = async (
    userId: string,
    newRole: "admin" | "support_admin" | "user",
  ) => {
    try {
  ...
  const counts = {
    all: allUsers.length,
    active: allUsers.filter((u) => u.status === "active" && u.role !== "admin")
      .length,
  ...
  return (
    <div
      className="max-w-3xl mx-auto space-y-5 page-enter sky-page"
      data-sky-page="admin-users"
    >
      {suspendModal && (
        <SuspendModal
          user={suspendModal}
      ...
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-0.5">
          Manage accounts, credits, roles, and view IP addresses.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-white/[0.06] overflow-hidden">
          {filtered.length === 0 ? (
            <div className="py-14 text-center text-[13px] text-slate-600">
              No users found.
            </div>
          ) : (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-slate-900 dark:text-white text-[13.5px] truncate">
                          {user.name}
                        </span>
                        {user.role === "admin" && (
                        ...
                        {isSuspended && (
                          <span className="inline-flex items-center text-[9.5px] font-bold px-1.5 py-0 rounded-full border text-red-400 border-red-500/20 bg-red-500/10">
                            Suspended
                          </span>
                        )}
                      </div>
                    ...
                    <div className="hidden sm:flex items-center gap-4 shrink-0">
                      <div className="text-right">
                        <div className="text-[13px] font-bold font-mono text-slate-900 dark:text-white">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        className="h-7 w-7 flex items-center justify-center rounded-lg text-slate-700 hover:text-slate-300 hover:bg-white/[0.05] transition-all"
                        title="Copy user ID"
                      >
                        {copiedId === user.id ? (
                          <Check className="h-3 w-3 text-emerald-400" />
                      ...
                      <button
                        onClick={() => setLocation(`/admin/users/${user.id}`)}
                        className="h-7 px-2.5 rounded-lg border border-white/[0.08] bg-white/[0.03] text-[11.5px] font-semibold text-slate-400 hover:text-white hover:bg-white/[0.06] transition-all flex items-center gap-1"
                      >
