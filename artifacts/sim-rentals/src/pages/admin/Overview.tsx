...
function ReferralSettingsCard() {
  ...
  const save = async () => {
    ...
    if (isNaN(amt) || amt < 0 || amt > 100) {
      toast({ title: "Invalid bonus amount", variant: "destructive" });
      return;
    }
    if (isNaN(minDep) || minDep < 0) {
      toast({
        title: "Invalid minimum deposit amount",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Program ${enabled ? "enabled" : "disabled"}, $${amt.toFixed(2)} bonus per referral.`,
      });
    } catch {
      ...
      <div className="px-5 py-4 border-b border-white/[0.06] flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-[11px] text-slate-500 mt-0.5">
            Bonus amounts and deposit requirements
          </div>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            >
              ...
              {enabled
                ? "Users can share codes and earn bonuses"
                : "Referral codes are currently disabled for all users"}
            </div>
            <button
              onClick={save}
              disabled={saving}
  ...
  const saveLinks = () => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "Invite links updated and visible to all users.",
      });
    }, 400);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <Skeleton
              key={i}
              className="h-32 w-full rounded-2xl bg-white/[0.04]"
            />
          ))}
        </div>
    ...
    >
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1 text-[13px]">
          Platform metrics and configuration.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5 relative overflow-hidden group hover:border-white/[0.12] transition-colors"
          >
            <div
              className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity"
              style={{
                background:
                  "radial-gradient(ellipse at 100% 0%, rgba(59,130,246,0.04) 0%, transparent 70%)",
              }}
            />
            <div className="flex items-start justify-between mb-4">
              <div
                className={`h-9 w-9 rounded-xl border flex items-center justify-center shrink-0 ${stat.iconCls}`}
              >
            ...
            <div
              className={`text-[2rem] font-black leading-none mb-1 ${stat.valueCls}`}
            >
              {stat.value}
            </div>
            <div className="text-[11px] font-bold text-slate-500 uppercase tracking-widest leading-tight">
              {stat.label}
            </div>
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            {data.providerStatuses.map((provider) => (
              <div
                key={provider.name}
                className="flex items-start justify-between gap-3 p-4 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-white/[0.1] transition-colors"
              >
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.06] flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-slate-500 mt-0.5">
                Appear as buttons in the user sidebar
              </div>
            </div>
          </div>
          <div className="p-5 space-y-4">
                ...
                <div className="flex gap-2">
                  {discord && (
                    <a
                      ...
                      className="text-[11px] text-indigo-300 border border-indigo-400/20 bg-indigo-400/8 rounded-full px-3 py-1 hover:bg-indigo-400/15 transition-colors font-medium"
                    >
                      Discord ✓
                    </a>
                  )}
