...
type TxFilter =
  "all" | "deposit" | "rental" | "adjustment" | "pending" | "failed";
...
function TxModal({ tx, onClose }: { tx: Tx; onClose: () => void }) {
  ...
  const rows: { label: string; value: React.ReactNode }[] = [
    {
      ...
      value: (
        <span className="font-mono text-[11px] break-all text-slate-400 select-all">
          {tx.id}
        </span>
      ),
    },
    ...
    },
    {
      ...
      value: (
        <span
          className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-bold ${statusCls(tx.status)}`}
        >
          {tx.status}
        </span>
      ),
    },
    {
      ...
      value: (
        <span
          className={`font-mono font-bold text-[16px] ${isDeposit ? "text-emerald-400" : "text-white"}`}
        >
    ...
    },
    {
      label: "User",
      value: (
        <span className="text-slate-300 break-all text-[12px]">
          {tx.userEmail}
        </span>
      ),
    },
      ...
      ? [
          {
            label: "User ID",
            value: (
              <span className="font-mono text-[10.5px] text-slate-600 break-all select-all">
                {tx.userId}
              </span>
            ),
          },
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-sm rounded-2xl border border-white/[0.08] bg-[#070c1a] shadow-2xl overflow-hidden"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.05]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-white text-[14px] capitalize">
                {tx.type.replace(/_/g, " ")}
              </div>
          ...
          <button
            onClick={onClose}
            className="h-7 w-7 flex items-center justify-center rounded-lg text-slate-500 hover:text-white hover:bg-white/[0.06] transition-all"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="px-5 py-4 space-y-0.5">
          {rows.map(({ label, value }) => (
            ...
            >
              <span className="text-[12px] text-slate-600 shrink-0 w-28">
                {label}
              </span>
              <div className="text-right text-[12.5px] min-w-0 flex-1">
                {value}
              </div>
            </div>
          ))}
...
export default function AdminTransactions() {
  ...
  const stats = useMemo(() => {
    const deposits = allTx.filter(
      (t) => t.type === "deposit" || t.type === "credit_purchase",
    );
    ...
    return allTx.filter((tx) => {
      ...
      const matchFilter =
        filter === "all"
          ? true
          : filter === "deposit"
            ? tx.type === "deposit" || tx.type === "credit_purchase"
            : filter === "rental"
              ? tx.type === "rental"
              : filter === "adjustment"
  ...
  const filterTabs: Array<{ key: TxFilter; label: string; count: number }> = [
    ...
    },
    {
      key: "rental",
      label: "Rentals",
      count: allTx.filter((t) => t.type === "rental").length,
    },
    {
      key: "adjustment",
      label: "Adjustments",
      count: allTx.filter((t) => t.type === "credit_adjustment").length,
    },
    {
      key: "pending",
      label: "Pending",
  ...
  return (
    <div
      ...
      data-sky-page="admin-transactions"
    >
      ...
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-0.5">
          Platform-wide payment and credit activity.
        </p>
      </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                        ...
                        <span
                          className={`inline-flex items-center rounded-full border px-1.5 py-0 text-[10px] font-bold ${statusCls(tx.status)}`}
                        >
                          {tx.status}
                        </span>
                      </div>
                    ...
                    <div
                      className={`font-mono font-bold text-[14px] shrink-0 ${
                        isFailed
