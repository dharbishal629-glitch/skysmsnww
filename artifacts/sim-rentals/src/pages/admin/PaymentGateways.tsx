...
async function fetchCoinKeys(): Promise<CoinKey[]> {
  const res = await fetch(`${API_URL}/api/admin/coin-api-keys`, {
    credentials: "include",
  });
...
export default function AdminPaymentGateways() {
  ...
  const { data: coinKeys = [], isLoading: keysLoading } = useQuery({
    queryKey: ["admin-coin-keys"],
    queryFn: fetchCoinKeys,
  });
  ...
  const saveKey = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/admin/coin-api-keys/${keyForm.coin}`,
        {
          method: "PUT",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            merchantKey: keyForm.merchantKey,
      ...
      toast({
        title: "API key saved",
        description: `${keyForm.coin} merchant key updated.`,
      });
    } catch (err: any) {
      toast({
  ...
  const deleteKey = async (coin: string) => {
    try {
      await fetch(`${API_URL}/api/admin/coin-api-keys/${coin}`, {
        method: "DELETE",
        credentials: "include",
      });
      qc.invalidateQueries({ queryKey: ["admin-coin-keys"] });
  ...
  const removeBan = async (id: string) => {
    setDeletingBan(id);
    try {
      await fetch(`${API_URL}/api/admin/ip-bans/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      qc.invalidateQueries({ queryKey: ["admin-ip-bans"] });
  ...
  return (
    <div
      className="max-w-3xl mx-auto space-y-7 pb-10 sky-page"
      data-sky-page="admin-gateways"
    >
      {/* Header */}
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-0.5">
          Per-coin OxaPay API keys and IP bans
        </p>
      </div>
      ...
      <section>
        ...
        <p className="text-[12px] text-slate-600 mb-4">
          Assign per-coin OxaPay merchant keys. When a user selects a coin at
          checkout, its key is used. Falls back to the default env key.
        </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setKeyForm((f) => ({ ...f, enabled: !f.enabled }))
                  }
          ...
          {keysLoading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-[13px] text-slate-600">
                No per-coin keys configured — using default OxaPay key.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-white/[0.04]">
              {coinKeys.map((key) => {
                const meta = COIN_META[key.coin] ?? {
                  label: key.coin,
                  emoji: key.coin[0],
                ...
                return (
                  ...
                  >
                    <div className="h-9 w-9 rounded-xl border border-white/[0.08] bg-white/[0.03] flex items-center justify-center shrink-0">
                      <span className={`text-[16px] font-black ${meta.color}`}>
                        {meta.emoji}
                      </span>
                    </div>
          ...
          <div className="rounded-2xl border border-red-500/15 bg-red-500/[0.03] p-5 mb-4 space-y-3">
            <div className="space-y-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  setBanForm((f) => ({ ...f, ipAddress: e.target.value }))
                }
        ...
        <div className="rounded-2xl border border-white/[0.07] overflow-hidden bg-white/[0.02]">
          {bansLoading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-[13px] text-slate-600">
                No IP bans. The platform is open to all IPs.
              </p>
            </div>
          ) : (
            <div className="divide-y divide-white/[0.04]">
              {ipBans.map((ban) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-bold text-slate-900 dark:text-white font-mono">
                      {ban.ipAddress}
                    </div>
                    {ban.reason && (
