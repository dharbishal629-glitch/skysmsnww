...
> = {
  ...
  rental_cancelled: {
    ...
    darkBg: "dark:bg-red-900/20 dark:border-red-700/40",
  },
  rental_expired: {
    icon: Clock,
    ...
    darkBg: "dark:bg-slate-800/40 dark:border-slate-700",
  },
  sms_received: {
    icon: MessageSquare,
    label: "SMS Received",
    ...
    darkBg: "dark:bg-cyan-900/20 dark:border-cyan-700/40",
  },
  payment: {
    icon: CreditCard,
    label: "Payment",
    ...
    darkBg: "dark:bg-indigo-900/20 dark:border-indigo-700/40",
  },
  api_key_created: {
    icon: Key,
  ...
  api_key_deleted: {
    ...
    darkBg: "dark:bg-rose-900/20 dark:border-rose-700/40",
  },
  totp_enabled: {
    icon: Shield,
    label: "2FA Enabled",
  ...
  password_changed: {
    ...
    darkBg: "dark:bg-orange-900/20 dark:border-orange-700/40",
  },
};
...
function matchesFilter(event: ActivityEvent, filter: string) {
  ...
  if (filter === "rental")
    return event.type.startsWith("rental") || event.type === "sms_received";
  if (filter === "security")
    return [
      "totp_enabled",
      "totp_disabled",
  ...
  return [
    {
      id: "1",
      type: "login",
      description: "Signed in from web browser",
      createdAt: new Date(now.getTime() - 5 * 60000).toISOString(),
    },
    {
      id: "2",
      type: "rental_created",
      description: "Rented Telegram number (+1 415 …)",
      createdAt: new Date(now.getTime() - 18 * 60000).toISOString(),
    },
    {
      id: "3",
      type: "sms_received",
      description: "SMS code received: 481 624",
      createdAt: new Date(now.getTime() - 16 * 60000).toISOString(),
    },
    {
      id: "4",
      type: "rental_expired",
      description: "Rental expired after 20 minutes",
      createdAt: new Date(now.getTime() - 14 * 60000).toISOString(),
    },
    {
      id: "5",
      type: "payment",
      description: "Balance topped up +$10.00",
      createdAt: new Date(now.getTime() - 2 * 3600000).toISOString(),
    },
    {
      id: "6",
      type: "api_key_created",
      description: "API key 'Production Bot' created",
    ...
    },
    {
      id: "8",
      type: "login",
      description: "Signed in from mobile browser",
      createdAt: new Date(now.getTime() - 48 * 3600000).toISOString(),
    },
    {
      id: "9",
      type: "rental_created",
      description: "Rented WhatsApp number (+44 …)",
    ...
    },
    {
      ...
      description: "Telegram rental cancelled (refunded)",
...
export default function Activity() {
  ...
  return (
    <div
      className="max-w-2xl mx-auto space-y-5 pb-8 page-enter sky-page"
      data-sky-page="activity"
    >
      {/* Header */}
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {filtered.map((event, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className={`h-9 w-9 rounded-xl border flex items-center justify-center shrink-0 ${cfg.bg} ${cfg.darkBg}`}
                  >
      ...
      {!loading && filtered.length > 0 && (
        <p className="text-center text-[11px] text-slate-400">
          ...
          {filter !== "all"
            ? ` · filtered by "${FILTER_TYPES.find((f) => f.value === filter)?.label}"`
            : ""}
        </p>
      )}
    </div>
