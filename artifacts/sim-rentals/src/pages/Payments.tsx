...
function PaymentRow({ payment }: { payment: any }) {
  ...
  return (
    <div className="border-b border-slate-200 dark:border-white/[0.04] last:border-0">
      ...
      >
        {logo ? (
          <img
            ...
            className="h-8 w-8 rounded-full shrink-0 bg-slate-100 dark:bg-white/[0.04]"
            onError={(e) => {
      ...
      <div
        className={`overflow-hidden transition-all duration-300 ease-in-out ${open ? "max-h-72" : "max-h-0"}`}
      >
            ...
            {[
              {
                ...
                value: (
                  <span className="font-mono text-[10.5px] text-slate-500 dark:text-slate-400 select-all break-all">
                    {payment.id}
                  </span>
                ),
              },
              ...
              <div
                key={label}
                className="flex items-start justify-between px-3.5 py-2.5 gap-3"
              >
...
export default function Payments() {
  ...
  const handleCustom = () => {
    ...
    if (!n || n < 0.5 || n > 1000) {
      toast({
        ...
        description: "Enter an amount between $0.50 and $1,000.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div
      className="max-w-xl mx-auto space-y-5 page-enter sky-page"
      data-sky-page="payments"
    >
      {/* Header */}
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-0.5">
          Add funds to your SKY SMS account.
        </p>
      </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-white/[0.05] bg-slate-50 dark:bg-white/[0.025] px-4 py-3">
            ...
            <p className="text-[12.5px] text-slate-600 dark:text-slate-400">
              Your balance will be credited instantly after payment.
            </p>
          </div>
          ...
          <div className="flex items-center gap-1 rounded-xl border border-slate-200 dark:border-white/[0.06] bg-slate-50 dark:bg-white/[0.02] p-0.5">
            {filters.map(({ key, label }) => (
              ...
              >
                ...
                {counts[key] > 0 && (
                  <span
                    className={`ml-1 text-[10px] ${filter === key ? "text-slate-400" : "text-slate-400 dark:text-slate-600"}`}
                  >
                    {counts[key]}
                  </span>
                )}
      ...
      <p className="text-[11.5px] text-slate-500 leading-relaxed">
        Payments are processed via crypto (OxaPay). Unused balance stays in your
        account.{" "}
        <a
          href="/refund-policy"
          className="text-slate-400 hover:text-slate-700 dark:hover:text-white transition-colors underline underline-offset-2"
        >
          Refund Policy
        </a>
      </p>
