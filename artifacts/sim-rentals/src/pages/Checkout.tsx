...
export default function Checkout() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `${API_URL}/api/payments/checkout-session/${sessionId}`,
        {
          credentials: "include",
        },
      );
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Redirecting to OxaPay for $${charged.toFixed(2)}.`,
      });
  ...
  if (sessionError || !session) {
    return (
      <div className="max-w-md mx-auto py-16 text-center">
        ...
        <h2 className="text-[16px] font-bold text-slate-900 dark:text-white mb-2">
          Checkout Expired
        </h2>
  ...
  return (
    <div
      className="max-w-md mx-auto pb-10 page-enter sky-page"
      data-sky-page="checkout"
    >
      {/* Header */}
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-white/[0.02] overflow-hidden mb-4 shadow-sm dark:shadow-none">
        ...
        <div className="p-5 space-y-3">
          ...
          {discount > 0 && (
            <div className="flex items-center justify-between">
              <span className="text-[13px] text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                <Tag className="h-3 w-3" /> Coupon discount
              </span>
        ...
        {coupon ? (
          <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/[0.07] border border-emerald-300 dark:border-emerald-500/20">
            ...
            <div className="flex-1 min-w-0">
              <span className="font-mono font-bold text-emerald-700 dark:text-emerald-300 text-[13px]">
                {coupon.code}
              </span>
      ...
      <p className="text-center text-[11px] text-slate-500 leading-relaxed">
        You'll be redirected to OxaPay to complete payment. Once confirmed, your
        ${amount.toFixed(2)} balance updates instantly.
      </p>
    </div>
  );
}
