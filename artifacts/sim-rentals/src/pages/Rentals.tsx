...
const statusStyles: Record<string, { label: string; cls: string }> = {
  active: {
    label: "Active",
    cls: "text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-700/40 bg-emerald-50 dark:bg-emerald-900/20",
  },
  completed: {
    label: "Expired",
    cls: "text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40",
  },
  sms_received: {
    label: "SMS ✓",
    cls: "text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-700/40 bg-emerald-50 dark:bg-emerald-900/20",
  ...
  return (
    <div className={`rounded-xl border overflow-hidden ${c.border} ${c.bg}`}>
      <div className="flex items-center justify-between px-3.5 py-2.5">
        ...
        <span
          className={`font-mono text-[15px] font-bold tabular-nums ${c.text}`}
        >
...
function ActiveRentalCard({ rental }: { rental: any }) {
  ...
  return (
    <div
      ...
      style={{
        boxShadow:
          "0 1px 4px rgba(0,0,0,0.05), 0 4px 16px rgba(16,185,129,0.06)",
      }}
    >
      {/* Top accent bar */}
      ...
      <div className="p-4 space-y-3.5">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            <span className="inline-flex items-center rounded-full border px-2 py-0.5 text-[10.5px] font-semibold text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-700/40 bg-emerald-50 dark:bg-emerald-900/20">
              Active
            </span>
            <div className="text-[12px] font-bold text-slate-900 dark:text-white mt-1">
        ...
        <div className="flex items-center justify-between rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700/40 px-3.5 py-3">
          <div>
            ...
            {rental.phoneNumber ? (
              <div
                className="font-mono font-bold text-slate-900 dark:text-white text-[16px] tracking-wide"
        ...
        {hasMessages ? (
          <div className="space-y-2">
            {rental.messages.map((msg: any) => (
              <div
                key={msg.id}
                className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3.5"
                data-testid={`row-message-${msg.id}`}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[10.5px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
                    {maskSender(msg.sender)}
                  </span>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">
                    {format(new Date(msg.receivedAt), "HH:mm:ss")}
                  </span>
                </div>
                ...
                {msg.code && (
                  <div className="mt-2.5 pt-2.5 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                    <div>
                      <div className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wider mb-0.5">
                        Code
                      </div>
                      <span className="font-mono font-bold text-[17px] text-emerald-700 dark:text-emerald-400 tracking-[0.2em]">
                        {msg.code}
                      </span>
                    </div>
                    <button
              ...
              cancelMutation.mutate(
                { id: rental.id },
                {
                  onSuccess: () => {
                    ...
                    toast({
                      title: "Cancelled",
                      description: "Refund applied if eligible.",
                    });
                  },
                },
              )
    ...
    <div className="border-b border-slate-100 dark:border-slate-700/60 last:border-0">
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-slate-800 dark:text-slate-200 text-[13.5px] truncate">
              {rental.serviceName}
            </span>
            {hasCodes && (
          ...
          <div className="text-[11.5px] text-slate-400 dark:text-slate-500 mt-0.5 truncate">
            {rental.countryName} ·{" "}
            {format(new Date(rental.createdAt), "MMM d, yyyy · HH:mm")}
          </div>
        </div>
        <div className="flex items-center gap-2.5 shrink-0">
          <div className="text-right">
            ...
            <span
              className={`inline-flex items-center rounded-full border px-1.5 py-0 text-[10px] font-semibold ${st.cls}`}
            >
              {st.label}
            </span>
          </div>
          ...
          {rental.phoneNumber && (
            <div className="flex items-center justify-between rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 px-3.5 py-2.5">
              <div>
                ...
                <div
                  className="font-mono text-slate-900 dark:text-white text-[14px] font-semibold"
                  data-testid={`text-rental-number-${rental.id}`}
                >
                  +{rental.phoneNumber}
                </div>
      ...
      <div className="space-y-4 max-w-lg mx-auto">
        ...
        <div className="space-y-3">
          {[1, 2].map((i) => (
            <Skeleton
              key={i}
              className="h-56 rounded-2xl bg-slate-100 dark:bg-slate-800"
            />
          ))}
        </div>
      </div>
  ...
  if (error || !data) {
    return (
      <div className="text-center py-16 max-w-lg mx-auto">
        <h2 className="text-[15px] font-semibold text-slate-800 dark:text-slate-200 mb-1">
    ...
    >
      ...
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-0.5 text-[13px]">
          {data.rentals.length === 0
            ? "No rentals yet."
            : `${activeRentals.length} active · ${pastRentals.length} past`}
        </p>
      </div>
      ...
      {data.rentals.length === 0 && (
        ...
        >
          ...
          <h3 className="font-semibold text-slate-800 dark:text-slate-200 text-[14px] mb-1">
            No rentals yet
          </h3>
          <p className="text-[12.5px] text-slate-500 dark:text-slate-400 mb-5 max-w-[200px]">
